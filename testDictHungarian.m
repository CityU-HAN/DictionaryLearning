function [expectedCost, learnedCost] = testDictHungarian(init, genSizes, lambda, randomSeed)
    if isempty(genSizes)
        features = 8;
        samples = 20;
        nrAtoms = 8;
    ...
    [learnD, learnW, learnW0] = ...
        DictionaryLearning(genY, lambda, nrAtoms, 200, false, false, {}, {});
    
    genWSparsityLevel = sum(sum(genW==0)) / numel(genW)
    learnWSparsityLevel = sum(sum(learnW==0)) / numel(learnW)
